...
async function startLevel(title){
  title = title.trim();
  if (!title){ toast("Name this story first"); return; }
...
function renderLogin(){
  ...
  <div class="login"><div class="login-card card">
    ...
    <p class="sub">Gather your team, size each story together, and watch the votes swim into place.</p>
    ${setupBanner}
    <div class="field">
    ...
    <div class="login-foot">No passwords. Pick any username — nothing leaves your team's room.</div>
  </div></div>`;
}
...
function wireLogin(){
  ...
  const btn = document.getElementById("login-btn");
  const submit = () => doLogin(username?.value);
  if (username){
...
function renderLobby(){
  ...
  const tiles = ids.map(id => {
    ...
    <button class="game-tile" data-join="${id}">
      ...
      <div class="tile-meta">
        ...
        <span>·</span>
        <span>${levelCount} stor${levelCount===1?"y":"ies"}</span>
      </div>
  ...
  const list = ids.length ? `<div class="game-grid">${tiles}</div>` : `
    <div class="empty-state">
      ...
      <p>Name your game, then lead your team through sizing each story together.</p>
    </div>`;
...
function renderLevelPanel(creator, cur, votes, playerEntries){
  ...
  if (!cur){
    if (creator){
      ...
      <div class="level-panel card">
        ...
        <h3 class="level-title placeholder">Name a story to open voting</h3>
        <div class="create-row">
          <input id="level-title" maxlength="80" placeholder="Item 12345" />
          <button class="btn" id="open-level">Open story</button>
        </div>
        <p class="deck-hint" style="margin-top:14px">Each story you open lets every player vote with the deck. Reveal when ready.</p>
      </div>`;
    }
    return `
      <div class="level-panel card center">
        <div class="level-num">Standby</div>
        <h3 class="level-title placeholder">Waiting for ${esc(gameData.creatorName)} to open the next story…</h3>
        <p class="deck-hint">When a story opens, the deck appears here for you to vote.</p>
      </div>`;
  }
  ...
  if (creator){
    if (!cur.revealed){
      ...
      controls = `
        <button class="btn teal" id="next-level">Next story</button>
  ...
  <div class="level-panel card">
    <div class="level-label">
      <span class="level-num">Story ${cur.number}</span>
    </div>
...
function renderHistory(){
  ...
  <div class="history card">
    <h3>Stories so far</h3>
    <p class="hsub">Each completed story and its winning estimate.</p>
    <table class="levels">
      <thead><tr><th>#</th><th>Story</th><th class="c-pts">Points</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
  </div>`;
}
...
function renderSide(playerEntries, creator){
  ...
  const hint = creator
    ? `<div class="side-card card">
         ...
         <p class="tip">Name a story to open voting for everyone.</p>
         <p class="tip">Hit <b>Reveal</b> to drop the votes into swimlanes. The fullest lane wins.</p>
         <p class="tip"><b>Next story</b> saves the result and moves on. <b>End game</b> shows the full table.</p>
       </div>`
    : `<div class="side-card card">
         <h4>How to play</h4>
         <p class="tip">When a story opens, tap a card to vote.</p>
...
function renderFinal(){
  ...
  <div class="overlay">
    <div class="final-card card">
      ...
      <p class="fsub">${levels.length} stor${levels.length===1?"y":"ies"} estimated · ${total} points total</p>

      <table class="levels">
        <thead><tr><th>#</th><th>Story</th><th class="c-pts">Points</th></tr></thead>
        <tbody>${rows||`<tr><td colspan="3" class="muted center" style="padding:24px">No stories were completed.</td></tr>`}</tbody>
      </table>
